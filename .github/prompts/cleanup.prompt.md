---
agent: 'agent'
model: 'GPT-5.2' 
tools: ['edit', 'execute/getTerminalOutput', 'execute/runInTerminal', 'read', 'search', 'web', 'todo']
description: 'Cleanup LaTeX source code'
---
Your goal is to cleanup the source code (macros, environments, styles, ...) of the provided (attached) LaTeX source file. Note that the compiled PDF should not change drastically, wording should stay the **exact** same. However, do correct simple typos.

Requirements:
* It is absolutely essential that you work in **small steps** (each step should equal exactly one frame). `\framebreak`s should be changed to multiple frames with the same title.
* Delete old, commente -out frames and similar legacy code; however, if the comment is R code that produces a figure **that is still in use**, convert this to an R script (`rsrs` folder).
* Do NOT use `vbframe`, instead use `framei` or `framev`.
* Do NOT use `\includegraphics`, instead use our custom image helpers.
* For columns, use `\splitV{}` and its derivatives.
* Prefer setting the font size (e.g., `small`) after the `frame` command, e.g., `\begin{framei}[fs=small]{...}`.
* Do NOT use `\bigskip`, `\smallskip`,, `\vspace` etc., for spacing. Use `\vfill`, or `\spacer` only - don't overcomplicate whitespace control!
* Use `\sourceref` and `\furtherreading` for citing, however, do NOT convert links to Google docs (those just denoted an internal presentation that the figure is taken from, and should remain as comments).
* Do NOT use `\small`, `\footnotesize`, etc., instead prefer `\begin{small} ... \end{small}`, `\begin{footnotesize} ... \end{footnotesize}` etc.
* Use `$$ ... $$` to denote display math, not `\[ \]`, and similarly `$ ... $` for inline math.
* Lastly, and most importantly, **you MUST follow the rules as defined in the [wiki](../../lecture_service.wiki/Slides.md)!**
