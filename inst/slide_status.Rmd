...
if (length(audit_results) > 0) {
  ...
  overview <- tibble::tibble(
    key = names(audit_results),
    Lecture = vapply(strsplit(names(audit_results), "/"), `[`, character(1), 1),
    Chapter = vapply(strsplit(names(audit_results), "/"), `[`, character(1), 2),
    Figures = audit_count("figures", safe_nrow),
    Orphaned = audit_count("orphaned_figures"),
    Attic = audit_count("attic_figures"),
    Missing = audit_count("missing_figures", safe_nrow),
    `Figs (man)` = audit_count("figures_man", safe_nrow),
    `Orphaned (man)` = audit_count("orphaned_figures_man"),
    `Missing (man)` = audit_count("missing_figures_man", safe_nrow)
  )
