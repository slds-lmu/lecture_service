...
# Filter to chapters that have rsrc/, figure/, or figure_man/
has_content <- vapply(
  chapter_names,
  function(ch) {
...
for `r lecture`. Each chapter's `rsrc/` scripts, `figure/` and `figure_man/`
files, and slide `.tex` files are cross-referenced to identify:

- **Orphaned figures**: present in `figure/` or `figure_man/` but not referenced by any slide
- **Missing figures**: referenced by slides but not present in `figure/` or `figure_man/`
...
overview <- tibble::tibble(
  ...
  Scripts = audit_count("scripts", safe_nrow),
  Figures = audit_count("figures", safe_nrow),
  `Orphaned Figs` = audit_count("orphaned_figures"),
  `Attic Figs` = audit_count("attic_figures"),
  `Missing Figs` = audit_count("missing_figures", safe_nrow),
...
if (params$run) {
  ...
  overview$`Orphaned Scripts` <- audit_count("orphaned_scripts")
}
...
for (ch in chapter_names) {
  ...
  cat(sprintf(
    "\n## %s\n\n%d scripts, %d figures, %d figure_man, %d slides\n\n",
    ch,
    n_scripts,
    n_figures,
    n_figures_man,
  ...
  # --- Attic figures (figure/attic/): parked, listed for cleanup awareness ---
  if (length(res$attic_figures) > 0) {
    attic_tbl <- tibble::tibble(Figure = res$attic_figures)
    tab <- attic_tbl |>
      kableExtra::kbl(
        format = "html",
        escape = FALSE,
        caption = sprintf(
          "%d figure(s) in %s/figure/attic/ (parked)",
          length(res$attic_figures), ch
        )
      ) |>
      kableExtra::kable_styling(
        full_width = FALSE,
  ...
  # --- Missing figures (figure/): referenced by slides but not on disk ---
  ...
  # --- figure_man/ table: one row per figure on disk, with status columns ---
  figs_man_no_attic <- res$figures_man[
    !startsWith(res$figures_man$figure_file, "attic/"),
  ]
  if (nrow(figs_man_no_attic) > 0) {
    fig_man_tbl <- tibble::tibble(
      Figure = figs_man_no_attic$figure_file,
      `Used by slide` = vapply(
        figs_man_no_attic$figure_base_name,
        function(fig) {
          slides <- names(res$slide_refs_man)[vapply(
            res$slide_refs_man,
            function(refs) fig %in% refs,
            logical(1)
          )]
    ...
    tab <- fig_man_tbl |>
      kableExtra::kbl(
        format = "html",
        escape = FALSE,
        caption = sprintf("Figures in %s/figure_man/", ch)
      ) |>
      kableExtra::kable_styling(
        full_width = TRUE,
        fixed_thead = TRUE,
  ...
  if (length(res$attic_figures_man) > 0) {
    attic_man_tbl <- tibble::tibble(Figure = res$attic_figures_man)
    tab <- attic_man_tbl |>
      kableExtra::kbl(
        format = "html",
        escape = FALSE,
        caption = sprintf(
          "%d figure(s) in %s/figure_man/attic/ (parked)",
          length(res$attic_figures_man), ch
        )
      ) |>
      kableExtra::kable_styling(
        full_width = FALSE,
  ...
  # --- Missing figures (figure_man/): referenced by slides but not on disk ---
  if (nrow(res$missing_figures_man) > 0) {
    ...
    tab <- miss_man_tbl |>
      kableExtra::kbl(
        ...
        escape = FALSE,
        caption = sprintf(
          "%d figure(s) referenced by slides but not in figure_man/",
          nrow(miss_man_tbl)
        )
      ) |>
      kableExtra::kable_styling(
