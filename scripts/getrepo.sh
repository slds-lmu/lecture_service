...
BRANCH=$(curl -sL https://api.github.com/repos/slds-lmu/$REPO | jq -r '.default_branch')
# Retrieve the contents of the repository and save them in $OUT
curl -sL https://api.github.com/repos/slds-lmu/$REPO/tarball/"$BRANCH" | tar -xz --directory=$OUT --strip-components=1
