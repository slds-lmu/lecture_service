...
# Adjust this as needed for the large multi-lecture overview. 
# If this is run within a single lecture's workflow it's ignored.
included_lectures <- c(
  "lecture_i2ml", 
  "lecture_advml", 
  "lecture_sl", 
...
if (!file.exists("slide_check_cache.rds")) {
  stop(paste(
    ...
    "Rscript -e 'source(here::here(\"helpers.R\")); check_all_slides_parallel()", sep = "\n "
  ))
}
...
this_repo_status() |>
  ...
  kableExtra::kbl(
    ...
    escape = FALSE, caption = "Status of the slide checker repo (time in UTC)"
  ) |>
...
lecture_status_local(lectures) |>
  ...
  dplyr::mutate(
    lecture_link = sprintf("<a href='https://slds-lmu.github.io/%s'>%s</a>", lecture, lecture),
    branch = sprintf("<a href='https://github.com/slds-lmu/%s/tree/%s'>%s</a>", lecture, branch, branch)
  ) |>
...
If you're missing a lecture here, you might have to add them to `included_lectures` in `slide_status.Rmd` (L21) and/or ensure they are downloaded correctly.
...
for (current_lecture in lectures) {
  ...
  for (current_topic in sort(unique(tmp$topic))) {
    ...
    tmp |>
      dplyr::select(
        slide_name, 
        compile_check, compare_check, compare_check_note) |>
      kableExtra::kbl(
